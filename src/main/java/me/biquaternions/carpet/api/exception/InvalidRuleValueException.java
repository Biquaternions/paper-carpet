package me.biquaternions.carpet.api.exception;

import io.papermc.paper.command.brigadier.CommandSourceStack;
import me.biquaternions.carpet.api.settings.CarpetRule;
...
public class InvalidRuleValueException extends Exception {
    ...
    public void notifySource(String ruleName, CommandSourceStack source) {
        if (this.getMessage() != null) {
            source.getSender().sendMessage(Component.text("Couldn't set value for rule " + ruleName + ": " + getMessage(), NamedTextColor.WHITE));
        }
    }
